...
<button onclick="runScan()">Run Buyer Scan</button><a href="/eval?v=from-dashboard" style="display:block;margin-top:10px;color:var(--text);text-decoration:none;font-weight:700;">Evaluation Console →</a>
...
<div class="box"><div class="eyebrow">Scan Summary</div><h3 id="scanMessage">Ready</h3><p class="muted">Source: USAspending award history. Active solicitations require SAM.gov validation.</p><p class="muted"><b>Scoring:</b> Evidence-based account ranking.</p><p id="filterStats" class="muted"></p></div>
...
<body>
  ...
  <script>
    ...
    async function runSelfTest() {
      ...
      const response = await fetch("/api/eval/self-test-v3?ts=" + Date.now(), { cache: "no-store" });
...
# --- HOSSAGENT FEDERAL REGISTER SOURCE 2 ---
